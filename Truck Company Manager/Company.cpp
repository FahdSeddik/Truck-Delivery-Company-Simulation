...
void Company:: ReadFile(ifstream & inputFile, string filename)
{
	...
	int CN, CS, CV;//check up duration in hours for each type of truck
	...
	if (inputFile.is_open()) // checks if file has been opened successfully and ready to be read from
	{
		...
		inputFile >> CN >> CS >> CV >> J; // reads check up duration in hours for each type of truck & number of trips before need for check-up from input file
		// create the specified trucks based on read info
		for (int i = 0; i < N; i++)
		{
			Truck* T = new Truck(NTC, CN, NS, J, NT);
			Avail_NT.enqueue(T);
		}

		for (int i = 0; i < S; i++)
		{
			Truck* T = new Truck(STC, CS, SS, J, ST);
			Avail_ST.enqueue(T);
		}

		for (int i = 0; i < V; i++)
		{
			Truck* T = new Truck(VTC, CV, VS, J, VT);
			Avail_VT.enqueue(T);
		}
		...
		for (int i = 0; i < n; i++)
		{
			...
			switch (State) // decides how to read the info of the event based on the type of the event
			{
			case('R'):
				char TYP;
				int DIST;
